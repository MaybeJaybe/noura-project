...
const Stretch2 = () => {
  ...
  const data_number = location?.state;
  const final_number = parseInt(data_number, 10);
  // // const data_minute = location?.state?.minute;
  const [workDuration, setWorkDuration] = useState(final_number);
  {
    console.log(typeof final_number);
  }
  const [minutes, setMinutes] = useState(final_number);
  ...
  return (
    <div className="stretch2-container">
      <p className="stretch2-prompt">Minutes until it’s time to move.</p>
      <div className="next-countdown">
        <div className="stretch-cont">
          <div className="timer">
          ...
          <div className="start">
            {!isRunning ? (
              <button className="pause-btn" onClick={startTimer}>
                Start
              </button>
            ) : (
